...
public:
	...
	{
		string suff = suffix(strFilePath);
		_suffnum = suff.size();
		...
		fwrite(suff.c_str(), suff.size(), 1, fout);
		fclose(finf);
		fclose(fout);
		delete[] pReadbuff;
	}
	...
	void UNCompressfile(const string& strFilePath)
	{
		...
		//从末尾获取文件后缀
		char suff[6];
		fseek(finF, 0 - _suffnum, SEEK_END);
		fread(suff, _suffnum, 1, finF);
		ULL filesize = 0;
		fseek(finF, 0-_suffnum - sizeof(filesize), SEEK_END);
		fread(&filesize, sizeof(filesize), 1, finF);

		size_t flagsize = 0;
		fseek(finF, 0 - _suffnum - sizeof(filesize)-sizeof(flagsize), SEEK_END);
		fread(&flagsize, sizeof(flagsize), 1, finF);
		fseek(finF, 0 - _suffnum - sizeof(flagsize)-sizeof(filesize)-flagsize, SEEK_END);
		
		//命名输出文件名称
		string filename = "Uncompress.";
		for (int i = 0; i<_suffnum ; i++)
		{
			filename.push_back(suff[i]);
		}
		FILE* fout = fopen(filename.c_str(), "wb");
		assert(fout);
...
private:
	...
	{
		string suff = filepath;
		string::iterator it = suff.end() - 1;
		int suffnum = 0;
		while (*it != '.')
		{
			it--;
		...
		string suf = suff.substr(suff.size() - suffnum, suffnum);
		return suf;
	}
